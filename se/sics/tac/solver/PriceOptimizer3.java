...
public class PriceOptimizer3 {
  ...
  */
	...
	if ((price = deltaPrices[IN_FLIGHT_PRICE + inday][inF]) != SUP) {
	  ...
	  if (inScore > bound) {
		...
		if (outScore > bound) {
		  ...
		  if ((outScore - totGPrice) > bound || 
		      (outScore - totCPrice) > bound) {
			...
			e1score = outScore - price;
			useE1 = true;
		      } else {
			// No entertainment - no buying either...
			e1score = outScore - prefs[3];
			useE1 = false;
		      }
		      if (e1score > bound) {
			for (int e2d = inday; e2d <= stop; e2d++) {
			  if (e2d == stop || e1d != e2d) {
			    ...
			    if (e2score > bound) {
			      for (int e3d = inday; e3d <= stop; e3d++) {
				if (e3d == stop || (e1d != e3d) && (e2d != e3d)) {
				  ...
				  if (newScore > bound) {
				    ...
				    // HOTEL CHECK!!!
				    // Entertainment done - check the hotels
				    long newerAlloc;
				    int newerScore;
  ...
  public static void main (String[] args) {
    PriceOptimizer3 po = new PriceOptimizer3();
