...
//     0x15
  ...
  */
  ...
  int scoreClientDays[][] = new int[8][4];
...
// 			   prices[E3_PRICE + i][e3]);
    ...
    } else if (allocPos % 2 == 0) { // < MAX_FLIGHT_POS) {
	...
	if ((price = deltaPrices[IN_FLIGHT_PRICE + inday][inF]) != SUP) {
	  int inScore;
	  if (inday > prefI) {
	    penalty = (inday - prefI) * 100 + price;
	  } else {
	    penalty = (prefI - inday) * 100 + price;
	  }
	  inScore = hscore - penalty;
	  if (inScore > bound) {
	    for (int j = 0; j < 4; j++) {
		...
		int outScore;
		if (outday > prefO) {
		  outScore = inScore - (totPen += (outday - prefO) * 100 + price);
		} else {
		  outScore = inScore - (totPen += (prefO - outday) * 100 + price);
		}
		if (outScore > bound) {
		  long bits = 0L;
		  ...
		  int scd = scoreClientDays[user][outday - inday];
		  int hotPen = 0;

		  if (totGPrice <= totCPrice) {
		    newAlloc = alloc | (1L << (HOTEL_ALLOC_POS + user8));
		    newScore = outScore - totGPrice;
		    newStuff1 = outStuff1 + (tmpStuff << GOOD_POS);
		   
		    if ((totPen + totGPrice) < scd && newScore > bound) {
		    ...
		    // Check other hotel
		    newScore = outScore - totCPrice;
		    newAlloc = alloc;
		    newStuff1 = outStuff1 + (tmpStuff << CHEAP_POS);
		    hotPen = totPen + totCPrice;
		  } else {
		    newScore = outScore - totCPrice;
		    newAlloc = alloc;
		    newStuff1 = outStuff1 + (tmpStuff << CHEAP_POS);

		    if ((totPen + totCPrice) < scd && newScore > bound) {
		      // Inling the entertainment stuff here!!!
		      bnb(newScore, newStay,
			  newAlloc, allocPos + 1,
			  newStuff1, stuff2, newFlights);
		    ...
		    newScore = outScore - totGPrice;
		    newStuff1 = outStuff1 + (tmpStuff << GOOD_POS);
		    hotPen = totPen + totGPrice;
		  }
		  if (hotPen < scd && newScore > bound) {
		    // Inling the entertainment stuff here!!!
		    bnb(newScore, newStay,
			newAlloc, allocPos + 1,
			newStuff1, stuff2, newFlights);
	...
	bnb(newScore, stay, alloc, allocPos + 1,
      ...
      int user = (allocPos / 2); // - MAX_FLIGHT_POS);
      int user8 = user * 8;
  ...
  void sortClients() {
    ...
    for (int i = 0; i < 8; i++) {
      int val = sortOrder[(preferences[i][0] << 2) |
			  preferences[i][1]];
      for (int j = i; j < 8; j++) {
	if (val > (tmpVal = sortOrder[(preferences[j][0] << 2) |
				      preferences[j][1]])) {
	  val = tmpVal;
