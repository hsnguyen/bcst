 ...
 * Updated : $Date: 2003/01/08 17:04:55 $
 *	     $Revision: 1.86 $
 * Purpose : AgentWare for Java
 *
 */
...
public class TACAgent implements Task, TACMessageReceiver {
  ...
  public final static String VERSION = "Beta 7";

  private static final Logger log =
    Logger.getLogger("se.sics.tac.aw.TACAgent");
  ...
  public TACAgent(AgentImpl agent, ArgEnumerator a, Properties config) {
    ...
    userName = a.getArgument("-agent", config.getProperty("agent", null));
    password = a.getArgument("-password",
    ...
    host = a.getArgument("-host", config.getProperty("host", "localhost"));
    port = a.getArgument("-port", getInt(config, "port", 6500));
    connectionClassName =
      a.getArgument("-connection",
		    config.getProperty("connection",
				       "se.sics.tac.aw.TACReader"));

    gameType =
      a.getArgument("-gameType", config.getProperty("gameType", null));
    ...
    this.logPrefix =
      a.getArgument("-logPrefix", config.getProperty("logPrefix", "aw"));
  ...
  public void performWork(long time, Object key, Object value) {
    ...
    } else if (key == "nextGame") {
      if (exitAfterGames < 0 || gamesPlayed < exitAfterGames) {
	td.addTask(time + 30000, key, null, this);
	if (playingGame == -1 && nextGameID == -1) {
	  requestNextGame();
	}
      }
	...
	requestQuotes((TACConnection) value);
      }
  ...
  void reset() {
    log.warning("performing connection reset");
    cancelTimers();
    // Clear transaction q
    transActionsNum = 0;
    waitActionsNum = 0;
  ...
  private void requestQuotes(TACConnection conn) {
...
//     }
  ...
  private void handleServerTime(TACMessage msg) {
	...
	if (!isNextGameTaskRunning) {
	  isNextGameTaskRunning = true;
	  TimeDispatcher.getDefault().addTask(serverTime + 30000,
					      "nextGame", null, this);
	}
      }
    }
...
//     LogFormatter.setLevelForAllHandlers(logLevel);
  ...
  public static void main(String[] args) {
    ...
    //String agentClass =
    //  a.getArgument("-agentimpl",
	//	    config.getProperty("agentimpl",
	//			       "se.sics.tac.aw.DummyAgent"));
    String agentClass = 
    		a.getArgument("-agentimpl", 
    				config.getProperty("agentimpl",
    						"se.sics.tac.sicsagent.SICSAgent"));
